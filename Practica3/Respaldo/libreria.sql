...
/*!50001 VIEW `Coordinador` AS select `L`.`ISBN` AS `ISBN`,`L`.`Nombre` AS `Nombre`,`L`.`Autor` AS `Autor`,`L`.`Editorial` AS `Editorial`,`L`.`Precio` AS `Precio`,`L`.`Portada` AS `Portada`,`P`.`idPedido` AS `idPedido`,`P`.`fecha` AS `fecha`,`P`.`idUsuario` AS `idUsuario`,`P`.`idSesion` AS `idSesion` from (`Libro` `L` join `PedidosUltimaSesion` `P`) where (`L`.`ISBN` = `P`.`ISBN`) */;
...
-- Dump completed on 2019-03-10  3:42:02
